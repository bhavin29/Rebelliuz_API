...
// View UserFollow My Request
exports.myRequestView = function (req, res) {
    try
    {
           var errMessage = '';
            let aggregate_options = [];
    ...
    try
        {
            ...
            (err,userFollow)=>{
            if (err){
                errMessage = '{ "UserFollow": { "message" : "Follow user is not saved!!"} }';
            ...
            if (!userFollow) {
                ...
                //Save and check error
                    ...
                    if (err)
                    {
                        errMessage = '{ "UserFollow": { "message" : "Follow user is not save"} }';
                        requestHandler.sendError(req,res, 422, 'Somthing went worng: ' + err.message,JSON.parse(errMessage));
                    }
                    else
                    {
                        requestHandler.sendSuccess(res,'Follow user save successfully.',200,userFollow);
                    }
                    });
                }
                else if (userFollow) {
                    ...
                    if (err){
                           errMessage = '{ "UserFollow": { "message" : "Follow user is not saved!!"} }';
                                    ...
                                    requestHandler.sendSuccess(res,'Follow user updated successfully.',200,userFollow);
                            }
                    });
               }
           });
