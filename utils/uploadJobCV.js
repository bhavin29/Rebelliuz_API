...
let storage = multer.diskStorage({
  destination: (req, file, cb) => {
     cb(null,  config.general.content_path + "/users/jobcv/");
    },
  filename: (req, file, cb) => {
    global.cv_filename = stringCode.generateString() + "_" + Date.now() + '.' + mime.extension(file.mimetype);
    console.log(global.cv_filename);
    cb(null, global.cv_filename);
  },
});
